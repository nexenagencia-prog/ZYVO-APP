...
export default function SkillsPage(){
  ...
  <section className={styles.board}>
    <article className={`${styles.card} ${styles.heroCard}`}><div className={styles.heroMedia} style={{backgroundImage:`url(${heroMedia})`,backgroundPosition:'82% 46%'}}/><div className={styles.heroShade} style={{background:'linear-gradient(90deg,rgba(4,16,25,.99) 0%,rgba(4,16,25,.96) 34%,rgba(4,16,25,.82) 52%,rgba(4,16,25,.28) 72%,rgba(4,16,25,.03) 100%),linear-gradient(0deg,rgba(2,11,18,.78),transparent 48%)'}}/><div className={styles.heroTop}><p>{hero.subtitle || FALLBACK_COPY.eyebrow}</p><h2>{hero.title || 'Skills'}</h2><h3>{hero.body || 'Inteligência que transforma suas reuniões em resultados.'}</h3></div><div className={styles.heroBottom}><i/><p>{typeof heroContent.insightsLabel==='string'?heroContent.insightsLabel:FALLBACK_COPY.insights}</p><strong>{typeof heroContent.insightsTitle==='string'?heroContent.insightsTitle:'Mais que reuniões. Evolução.'}</strong><button><span>▷</span>{typeof heroContent.ctaLabel==='string'?heroContent.ctaLabel:'Explorar agora'}</button></div></article>
